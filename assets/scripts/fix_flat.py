...
prods={p["slug"]:p for p in json.load(open("/home/user/someplace/assets/capsule.json"))}
CATVAR=json.load(open(f"{SCRATCH}/catalog_variants.json"))
TARGETS=["beach-dress","bodycon","one-piece"]
SLEEVE_PL={"sleeve_left","sleeve_right","inside_yoke"}
VISIBLE={"front","back","sleeve_left","sleeve_right","default","top_front","top_back",
 "top","bottom","bottom_front","bottom_back","leg_left","leg_right"}
def api(m,u,b=None):
...
paths=json.load(open(f"{OUT}/index.json"))
last=0
for slug in TARGETS:
    it=prods[slug]; cid=it["catalog_id"]; dm=dims(cid)
    ...
    body={"variant_ids":[vid],"format":"jpg","option_groups":["Flat"],"files":files}
    g=32-(time.time()-last)
    if g>0: time.sleep(g)
    ...
    for _ in range(8):
        ...
        res=(d or {}).get("result")
        if isinstance(res,dict) and res.get("task_key"): key=res["task_key"]; print("create",slug,key,flush=True); break
